...
void RTC_GetTime(RTC_TimeTypeDef* RTC_TimeStruct)
{
  ...
  RegTmp = CW_RTC->TIME;
  while(RegTmp != CW_RTC->TIME) {RegTmp = CW_RTC->TIME;}  // 连续两次读取的内容一致，认为读取成功
...
void RTC_GetDate(RTC_DateTypeDef *RTC_Date)
{
  ...
  RegTmp = CW_RTC->DATE;
  while(RegTmp != CW_RTC->DATE) {RegTmp = CW_RTC->DATE;}  // 连续两次读取的内容一致，认为读取成功
...
void RTC_GetAlarm(uint32_t RTC_Alarm, RTC_AlarmTypeDef* RTC_AlarmStruct)
{
  ...
  if (RTC_Alarm == RTC_Alarm_A)
  {
    ...
    while(RegTmp != CW_RTC->ALARMA) {RegTmp = CW_RTC->ALARMA;}
  }
  else
  {
    RegTmp = CW_RTC->ALARMB;
    while(RegTmp != CW_RTC->ALARMB) {RegTmp = CW_RTC->ALARMB;}
  }
